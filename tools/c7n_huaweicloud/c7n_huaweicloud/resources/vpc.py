...
class SecurityGroupRuleFilter(Filter):
    ...
    def __call__(self, resource):
        ...
        #if match:
        #    show_resource = self.data.get('show-resource', 'sg-rule')
        #    if show_resource == 'sg':
        #        query = {"id": [resource['security_group_id']]}
        #        sg_resource = self.manager.get_resource_manager('vpc-security-group').resources(query)
        #        log.info("Sg resource is %s", sg_resource)
        #        if sg_resource not in matched:
        #            matched.append(sg_resource)
        #    else:
        #        matched.append(resource)
            ...
            # remove rules with a list of rule filter conditions
                ...
                for f in mode:
                    try:
                        ...
                        log.info("to delete rules are %s", to_delete_rules)
                    except exceptions.ClientRequestException as ex:
